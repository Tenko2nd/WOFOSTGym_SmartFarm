...
from pcse_gym.args import WOFOST_Args, Agro_Args
import yaml
import numpy as np
...
def make_config(
    ...
    return {
        ...
        "OUTPUT_WEEKDAY": OUTPUT_WEEKDAY,
        "OUTPUT_VARS": OUTPUT_VARS,
        "SUMMARY_OUTPUT_VARS": SUMMARY_OUTPUT_VARS,
...
def set_agro_params(agromanagement: dict, args: Agro_Args):
    ...
    if args.crop_variety is not None:
        agromanagement["CropCalendar"]["crop_variety"] = args.crop_variety
...
class ParamLoader:
    ...
    def __init__(
        ...
        try:
            ...
            msg = (
                f"Error loading State Ranges file `{base_fpath}{range_fpath}`. Check correct path or that file exists."
            )
            raise Exception(msg)
