...
from pcse_gym.envs.wofost_base import Multi_NPK_Env
...
class Pernnial_Multi_Limited_NPKW_Env(Multi_NPK_Env, LNPKW):
    ...
    def _take_action(self, action: int) -> tuple[float, float, float, float]:
        ...
        if action >= 3 * self.num_fert + 1:
            ...
            [
                self.models[i]._send_signal(signal=pcse.signals.irrigate, amount=i_amount, efficiency=self.irrig_effec)
        ...
        if (action - 1) // self.num_fert == 0:
            ...
            [
                self.models[i]._send_signal(
                    signal=pcse.signals.apply_npk, N_amount=n_amount, N_recovery=self.n_recovery
                )
                for i in range(self.num_farms)
            ]
            ...
            [
                self.models[i]._send_signal(
                    signal=pcse.signals.apply_npk, P_amount=p_amount, P_recovery=self.p_recovery
                )
                for i in range(self.num_farms)
            ]
            ...
            [
                self.models[i]._send_signal(
                    signal=pcse.signals.apply_npk, K_amount=k_amount, K_recovery=self.k_recovery
                )
                for i in range(self.num_farms)
            ]
        return (n_amount, p_amount, k_amount, i_amount)
...
class Pernnial_Multi_PP_Env(Multi_NPK_Env, PP):
...
class Pernnial_Multi_Limited_NPK_Env(Multi_NPK_Env, LNPK):
...
class Pernnial_Multi_Limited_N_Env(Multi_NPK_Env, LN):
...
class Pernnial_Multi_Limited_NW_Env(Multi_NPK_Env, LNW):
...
class Pernnial_Multi_Limited_W_Env(Multi_NPK_Env, LW):
...
class Pernnial_Multi_Limited_Layered_NPKW_Env(Multi_NPK_Env, LNPKW):
...
class Pernnial_Multi_Layered_PP_Env(Multi_NPK_Env, PP):
    """Simulates Potential Production. That is how much the crop would grow
...
class Pernnial_Multi_Limited_Layered_NPK_Env(Multi_NPK_Env, LNPK):
...
class Pernnial_Multi_Limited_Layered_N_Env(Multi_NPK_Env, LN):
...
class Pernnial_Multi_Limited_Layered_NW_Env(Multi_NPK_Env, LNW):
...
class Pernnial_Multi_Limited_Layered_W_Env(Multi_NPK_Env, LW):
